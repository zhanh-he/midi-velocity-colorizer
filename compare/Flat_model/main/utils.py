...
def precision(est_velo, gt_velo, threshold=0.1):
...
def recall(est_velo, gt_velo, threshold=0.1):
    diff = torch.abs(est_velo - gt_velo)
    tp = torch.sum(diff < threshold).float()       # True positives
...
def f1_score(est_velo, gt_velo, threshold=0.1):
    p = precision(est_velo, gt_velo, threshold)
    r = recall(est_velo, gt_velo, threshold)
...
class TestEveryNEpochs(Callback):
    ...
    """
    def __init__(self, test_dataloader, every_n_epochs=2):
